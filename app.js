...
document.querySelector('.btn-roll').addEventListener('click' , function(){
	...
	if(gamePlaying)//as gamePlaying is already true or false, we only need the gamePlaying variable and nothing else
	{
       ...
       diceDOM.style.display = 'block'; //use block to display something and none to hide 
       diceDOM.src = 'dice-' + dice + '.png'

       //3. Update the round score IF the rolled number is not a one
       if(dice !== 1){
    	//Add score
...
function nextPlayer(){
	//Making use of ternary operator
    	...
    	*/
    	...
    	document.querySelector('.dice').style.display = 'none'; //Hiding the dice again when either one of the player rolls 1 and it's another player's turn
...
function init(){
    ...
    document.querySelector('.dice').style.display = 'none';//Changing css style, in this case hiding the dice before the game begins
